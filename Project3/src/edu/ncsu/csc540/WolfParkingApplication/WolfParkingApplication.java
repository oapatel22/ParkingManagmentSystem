...
public class WolfParkingApplication {
	...
	public static void main(final String[] args) {
		...
		java.sql.Date date = new java.sql.Date(today.getTime()); // your SQL
																	// date
																	// object
		...
		// enterCitation( date, 10, "Unpaid", time, 1, "Invalid Permit", "LotName 1",
		// "LicenseNum 1" );
		// updateCitation( 1, "Expired Permit", 50 );
		// enterCitation( date, 10, "Unpaid", time, 2, "Invalid Permit", "LotName 1",
		// "LicenseNum 1" );
		// deleteCitation( 2 );
		// NEED TO CHECK // FIX
	...
	// TODO CAN YOU UPDATE PRIMARY KEYS?
	...
	/**
	 * Updates an existing Space with the given spaceNumber and lotName to the given
	 * parameters
	 *
	...
	public static void updateSpace(final int spaceNumber, final String lotName, final String spaceType,
		...
		try {
			// Updates Space entry
			statement.executeUpdate("UPDATE Space SET SpaceType='" + spaceType + "', AvailabilityStatus='"
					+ availabilityStatus + "' " + "WHERE SpaceNumber='" + spaceNumber
					+ "' AND WHERE AvailabilityStatus='" + availabilityStatus + "';");
	...
	/**
	 * Assigns an existing Zone existing within a Parking Lot to another Parking Lot
	 *
	 *
	 ...
	 * @param lotName        the name fo the lot where the citation was issued
	 * @param licenseNum     the license plate on the citation issued
	 */
	public static void updatePaymentStatus(final String paymentStatus, final int citationNumber, final String lotName,
		...
		try {
			...
			statement.executeUpdate("UPDATE Citation SET PaymentStatus='" + paymentStatus + "' WHERE CitationNumber ='"
					+ citationNumber + "' AND LotName ='" + lotName + "' AND LicenseNum ='" + licenseNum + "';");
		} catch (SQLException e) {
			System.out.println("Error message");
		}
	}
	...
	// try {
	// ResultSet rs = statement.executeQuery("SELECT * FROM Driver WHERE DriverID='"
	// + driverID + "';" );
	...
	public static void getCitationReportByTime(final String StartDate, final String EndDate) {
		try {
			ResultSet rs = statement.executeQuery("SELECT p.LotName, COUNT(c.CitationNumber) AS TotalCitations FROM ParkingLot p LEFT JOIN Citation c ON p.LotName = c.LotName AND c.CitationDate BETWEEN '"
	...
	public static void getPermitInformation(final String DriverId) {
		try {
			ResultSet rs = statement.executeQuery(
					"SELECT * From Permit WHERE Permit.DriverId = '" + DriverId + "';");
			String s = "";
			StringBuilder sb = new StringBuilder(s);
	...
	public static void getAvailableSpaces(final String LotName) {
		try {
			ResultSet rs = statement.executeQuery(
					"SELECT SpaceNumber From Space WHERE spaceType = 'regular' AND lotName = '"
							+ LotName + "' AND AvailabilityStatus = 'Available';");
			String s = "";
			StringBuilder sb = new StringBuilder(s);
