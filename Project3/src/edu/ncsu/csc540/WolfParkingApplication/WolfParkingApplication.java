...
//                "2023-11-15", "06:00:00");
		...
		while (true) {
			...
			case "4":
				...
				switch (operation) {
				case "1":
					System.out.print("Enter start and end date of Citation in the format (YYYY-MM-DD, YYYY-MM-DD):");
	...
	public static void updateDriverInformation(final String driverID, final String status, final String name) {
		...
		if (name.length() != 0 || name != null) {
			try {
				...
				statement
						.executeUpdate("UPDATE Driver SET Name='" + name + "' " + "WHERE DriverID='" + driverID + "';");
			} catch (SQLException e) {
				System.out.println("Error message");
			}
		}
